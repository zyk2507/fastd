...
for _ in $(seq 1 120); do
	...
	if [[ -S "$WORK/a.status" && -S "$WORK/b.status" ]]; then
		"$FASTD" --status-socket "$WORK/a.status" --status --json > "$WORK/a.json" 2>/dev/null || true
		"$FASTD" --status-socket "$WORK/b.status" --status --json > "$WORK/b.json" 2>/dev/null || true

		if python3 - "$WORK/a.json" "$WORK/b.json" <<'PY'
import json
import sys
...
PY
			then
				...
				printf 'ok 1 - control relay establishes direct UDP path through full-cone mappings\n'
				exit 0
			fi
	fi
done
